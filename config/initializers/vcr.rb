if defined?(VCR) && Rails.env.test?
  VCR.configure do |c|
    ...
    c.filter_sensitive_data('<API_KEY>') { SECRETS['data_dot_gov']['primary_comment_api_key'] }
    c.filter_sensitive_data('<API_KEY>') { SECRETS['data_dot_gov']['secondary_comment_api_key'] }
