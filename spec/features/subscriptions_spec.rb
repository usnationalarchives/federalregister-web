...
feature "Subscriptions", :no_ci do
  ...
  context "receiving a subscription" do
    scenario "includes the proper documents and an unsubscribe link", :js do
      ...
      manage_subscriptions_link = "#{Settings.services.fr.web.base_url}/my/subscriptions?utm_campaign=utility_links&amp;utm_content=manage_subscription&amp;utm_medium=email&amp;utm_source=federalregister.gov"
      expect(current_email).to have_body_text(manage_subscriptions_link)

      unsubscribe_link = "#{Settings.services.fr.web.base_url}/my/subscriptions/(((token)))/unsubscribe"
